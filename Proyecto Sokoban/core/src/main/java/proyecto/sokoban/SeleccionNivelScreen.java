...
public class SeleccionNivelScreen implements Screen {
    ...
    public void show() {
        ...
        Label titulo = new Label(Textos.get("nivel.titulo"), skin);
        ...
        TextButton btnRegresar = new TextButton(Textos.get("nivel.regresar"), skin);
    ...
    private void crearBotonNivel(Table table, int nivel) {
        ...
        String texto = Textos.get("nivel.nivel") + nivel;
 
        if (gestor.getLoggedIn() != null && !gestor.getLoggedIn().nivelDesbloqueado(nivel)) {
            texto += " - "+Textos.get("nivel.bloqueado");
        }
    ...
    private void intentarAbrirNivel(int nivel) {
        ...
        if (usuario != null && !usuario.nivelDesbloqueado(nivel)) {
            Dialog dialog = new Dialog(Textos.get("nivel.bloqueado"), skin);
            dialog.text(Textos.get("nivel.noDesbloqueado"));
            dialog.button("Ok");
            dialog.show(stage);
            return;
        }
